...
- 등록일이 10~12월이고, 휴무일이 1~3월이면 → **반드시 다음 연도** 사용
  예) 등록일 2025-12-29 + 휴무일 "2월 1일" → **2026-02-01** (절대 2025-02-01 아님!)
  예) 등록일 2026-01-15 + 휴무일 "2월 15일" → **2026-02-15**
- 등록일보다 과거의 연도는 절대 사용하지 마세요!
- 헷갈리면: 등록일 이후의 가장 가까운 해당 날짜를 선택하세요.
- **자가 검증:** dates의 연월이 notice_date보다 과거이면 틀린 것입니다!
...
**★★★ 최우선 규칙: valid_month의 연도 결정 ★★★**
공지 등록일(notice_date)을 기준으로 프로그램 적용 월(valid_month)의 연도를 결정합니다:
- 등록일이 10~12월이고, 프로그램이 1~3월이면 → **반드시 다음 연도** 사용
  예) 등록일 2025-12-29 + 프로그램 "1월" → **2026년 1월** (절대 2025년 1월 아님!)
  예) 등록일 2025-11-15 + 프로그램 "2월" → **2026년 2월**
- 등록일보다 과거의 연도는 절대 사용하지 마세요!
- 헷갈리면: 등록일 이후의 가장 가까운 해당 월을 선택하세요.

**★★★ 두 번째 규칙: "일일자유이용"과 "월자유"를 반드시 구분하세요! ★★★**
...
**★★★ 세 번째 규칙: 수영장 운영 여부 확인 (매우 중요!) ★★★**

문서가 종합체육시설(볼링, 테니스, 빙상, 배드민턴 등 여러 종목 포함) 공지인 경우:
- 반드시 "수영", "수영장", "수영(25M)", "수영(50M)" 키워드가 포함된 행/열만 추출하세요
- 다른 종목(볼링, 테니스, 빙상, 스크린골프, 배드민턴, 헬스, 농구, 골프 등)의 스케줄은 **절대 추출하지 마세요!**

**수영장 휴장/미운영 감지:**
- 문서에 다른 종목 스케줄은 있으나 "수영" 관련 행이 없으면 → 수영장 미운영
- "수영장 휴장", "수영장 임시휴관", "수영장 미운영" 등의 문구가 있으면 → 수영장 휴장
- 스케줄 표에서 수영 행이 "정기휴장일", "휴장", "-" 등으로만 표시되면 → 수영장 미운영
- **이 경우 schedules를 빈 배열 []로 반환하세요!**

**수영장 미운영 시 응답:**
{
  "facility_name": "시설명",
  "valid_month": "YYYY년 M월",
  "schedules": [],
  "fees": [],
  "notes": ["수영장 미운영"]
}
...
- **수영이 아닌 다른 종목 스케줄** (볼링, 테니스, 빙상, 배드민턴, 헬스, 스크린골프, 농구 등)
...
2. valid_month: 프로그램이 적용되는 월 (YYYY년 M월 형식으로 반환)
   - **연도 추출 우선순위:**
     1순위: 제목(notice_title)에 "YYYY년 M월"이 있으면 그대로 사용
     2순위: 문서에 "YYYY년 M월"이 있으면 그대로 사용
     3순위: 등록일(notice_date) 기준으로 추론 ← **대부분 이 경우!**
   - **★★★ 연도 추론 핵심 (3순위 - 매우 중요!) ★★★:**
     * 원칙: 등록일 이후 가장 가까운 해당 월 = valid_month의 연도
     * **등록일이 10~12월 + 프로그램이 1~3월 → 무조건 다음 연도!**
       - 등록일 2025-12-29, "1월" → **2026년 1월** ✓ (정답)
       - 등록일 2025-12-29, "1월" → 2025년 1월 ✗ (이미 지난 달!)
       - 등록일 2025-11-15, "2월" → **2026년 2월** ✓
     * **등록일보다 과거 연월은 논리적으로 불가능!**
       - 공지는 미래의 프로그램을 안내하는 것이므로, valid_month >= 등록일
   - **자가 검증:** valid_month가 notice_date보다 과거이면 틀린 것입니다!
...
5. **검증 체크리스트**:
   - [ ] **추출한 스케줄이 "수영" 관련인가?** (볼링, 테니스, 빙상 등 다른 종목이 아닌지!)
   - [ ] **수영장이 실제로 운영 중인가?** (휴장/미운영이면 빈 schedules 반환!)
