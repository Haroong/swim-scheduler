...
중요 규칙 (반드시 준수):
1. 같은 시간대가 서로 다른 용량(capacity)으로 여러 번 나타나면, 각각 다른 요일에 속합니다.
   예) 16:00~17:50이 80명과 100명으로 나타나면 → 80명은 평일, 100명은 주말

2. 요일별 시간대 분리 원칙:
   - 시간대와 용량이 문서에서 어떤 요일 섹션에 속해있는지 정확히 파악하세요
   - 표 형식이 깨진 경우, "평일", "토요일", "일요일" 단어 위치를 기준으로 시간대를 분리하세요
   - 평일 섹션에 있는 시간대만 평일에, 토요일 섹션에 있는 시간대만 토요일에 할당하세요

3. 용량 차이로 요일 구분:
   - 같은 시간대(예: 16:00~17:50)가 용량만 다르게 반복되면 다음을 확인하세요:
     a) 용량이 작은 쪽이 평일, 큰 쪽이 주말일 가능성이 높음
     b) 문서 구조상 어느 요일 섹션에 속하는지 확인
     c) 의심스러우면 용량이 다른 시간대는 서로 다른 요일에 할당

4. 중복 방지:
   - 한 시간대를 여러 요일에 중복 할당하지 마세요
   - 특히 주말 시간대를 평일에 잘못 할당하지 않도록 주의하세요
